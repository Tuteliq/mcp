...
- **Harmonized annotation key order across admin tools.** `READ_ONLY`,
  `ADDITIVE`, and `DESTRUCTIVE` in `src/tools/admin.ts` now declare
  `readOnlyHint`/`openWorldHint`/`destructiveHint` in the same order as the
  inline annotation literals elsewhere in the codebase. Key order carries no
  protocol meaning, so this doesn't change behavior; it was the one objective
  discrepancy found while investigating an unexplained per-tool
  approval-gating report (no code in this stack implements approval gating,
  so that report was very likely client-side) and is harmonized since it's
  zero-risk.
